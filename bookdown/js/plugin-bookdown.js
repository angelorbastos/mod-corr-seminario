gitbook.require(["gitbook", "lodash", "jQuery"], function(gitbook, _, $) {
  ...
  gitbook.events.bind("start", function(e, config) {
    ...
    if (edit && edit.link) gitbook.toolbar.createButton({
      icon: 'fa fa-edit',
      label: edit.text || 'Editar no Github',
      position: 'left',
      onClick: function(e) {
        e.preventDefault();
    ...
    if (history && history.link) gitbook.toolbar.createButton({
      icon: 'fa fa-history',
      label: history.text || 'Histórico no Github',
      position: 'left',
      onClick: function(e) {
        e.preventDefault();
    ...
    var info = ['Atalhos do teclado:',
      'Teclas para esquerda e direita: navegar entre as páginas.',
      'Telca "s": ativar a barra lateral'];
    if (config.search !== false) info.push('Tecla "f": ativar a barra de pesquisa ' +
      '(use as teclas para cima, para baixo e Enter na barra de pesquisa para navegar entre os resultados; ' +
